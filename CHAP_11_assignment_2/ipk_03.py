...
result=StringVar(value='도형을 선택하고 그리기를 누르세요')
label=Label(root, textvariable=result).pack()

frame1=Frame(root)
frame1.pack()

RectangleRadio=Radiobutton(frame1, text='사각형', variable=choice, value=1, command=Rectangle.draw).pack(side='left')
CirclaRadio=Radiobutton(frame1, text='원', variable=choice, value=2, command=Circle.draw).pack(side='left')
...
def draw_shape():
    ...
    shape.draw(w)
    result.set(f'면적={shape.area():.2f},둘레={shape.perimeter():.2f}')
